...
auto showErrorIfDebug = [](const std::string &message) {
    if (DEBUG_MODE) {
        Alert::showError([NSString stringWithFormat:@"%s", message.c_str()]);
    }
};
...
namespace IL2CPP
{
    namespace Helper
    {
        ...
        void HookStaticMethod(const std::string &className, const std::string &methodName, int argsCount, Ret (*hookedFunction)(Args...), Ret (*originalFunction)(Args...)) 
        {
            ...
            if (!il2cppClass && DEBUG_MODE)
            {
                showErrorIfDebug("Class " + className + " not found");
                return;
            }

            uint64_t methodOffset = IL2CPP::Class::Utils::GetMethodPointerRVA(il2cppClass, methodName.c_str(), argsCount);
            if (!methodOffset && DEBUG_MODE)
            {
                showErrorIfDebug("Method " + methodName + " not found in class " + className);
                return;
            }
        ...
        void PatchStaticMethod(const std::string &className, const std::string &methodName, int argsCount, const std::string hexBytes) {
            auto il2cppClass = IL2CPP::Class::Find(className.c_str());
            if (!il2cppClass && DEBUG_MODE)
            {
                showErrorIfDebug("Class " + className + " not found");
            ...
            patchOffset(methodOffset, hexBytes);
        }
        ...
        T GetStaticFieldValue(const std::string &className, const std::string &fieldName)
        {
            ...
            if (!value)
            {
                showErrorIfDebug("Field " + fieldName + " not found in class " + className);
                return T();
            }
        ...
        T GetPropertyValue(Unity::CGameObject *obj, const std::string &className, const std::string &propertyName)
        {
            ...
            if (!instance)
            {
                showErrorIfDebug("Failed to get instance of class " + className);
                return T();
            }
        ...
        Unity::CGameObject *CloneGameObject(Unity::CGameObject *obj, const Unity::Vector3 &position)
        {
            ...
            if (!il2cppClass)
            {
                showErrorIfDebug("Class UnityEngine.Object not found");
                return nullptr;
            }
            ...
            {
                showErrorIfDebug("Method Instantiate not found in class UnityEngine.Object");
                return nullptr;
            }
            ...
            if (!clonedObj)
            {
                showErrorIfDebug("Failed to clone object");
                return nullptr;
            }
        ...
        Unity::CGameObject *FindGameObjectByName(const std::string &name)
        {
            ...
            if (!gameObject)
            {
                showErrorIfDebug( "GameObject " + name + " not found.\n");
            }
            return gameObject;
        }
        ...
        std::vector<Unity::CGameObject *> FindGameObjectsWithTag(const std::string &tag)
        {
            ...
            if (!gameObjectsArray)
            {
                showErrorIfDebug( "No GameObjects found with tag " + tag + ".\n");
                return gameObjects;
            }
        ...
        T *GetComponent(Unity::CGameObject *obj, const std::string &componentName)
        {
            ...
            if (!component)
            {
                showErrorIfDebug( "Component " + componentName + " not found in GameObject.\n");
                return nullptr;
            }
        ...
        std::vector<T *> GetComponents(Unity::CGameObject *obj, const std::string &componentName)
        {
            ...
            if (!componentsArray)
            {
                showErrorIfDebug( "No components found with name " + componentName + " in GameObject.\n");
                return components;
            }
        ...
        Ret CreateInstanceAndInvoke(const std::string &className, const std::string &ctorName, const std::string &methodName, Args... ctorArgs)
        {
            ...
            if (!ctorPointer)
            {
                showErrorIfDebug( "Constructor " + ctorName + " not found in class " + className + ".\n");
                return Ret();
            }
            ...
            if (!instance)
            {
                showErrorIfDebug( "Failed to create instance of class " + className + ".\n");
                return Ret();
            }
        ...
        std::vector<Unity::CGameObject *> FindAllInstancesByClass(const std::string &className)
        {
            ...
            if (!allObjects)
            {
                showErrorIfDebug( "No GameObjects found.\n");
                return instances;
            }
        ...
        Ret InvokeMethodOnComponent(Unity::CGameObject *obj, const std::string &componentName, const std::string &methodName, Args... args)
        {
            ...
            if (!il2cppClass)
            {
                showErrorIfDebug( "Class " + componentName + " not found.\n");
                return Ret();
            }
            ...
            if (!methodPointer)
            {
                showErrorIfDebug( "Method " + methodName + " not found in class " + componentName + ".\n");
                return Ret();
            }
        ...
        Unity::CGameObject *CreateInstance(const std::string &className)
        {
            ...
            {
                showErrorIfDebug( "Instantiate method not found in class " + className + ".\n");
                return nullptr;
            }
